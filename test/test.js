...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterRound10, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterRound10( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which rounds each iterated value to the nearest power of 10 on a linear scale', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterRound10( linspace( -200.0, 200.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( -200.0, 200.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = round10( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which rounds each iterated value to the nearest power of 10 on a linear scale (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': round10( values[ 0 ] ),
			'done': false
		},
		{
			'value': round10( values[ 1 ] ),
			'done': false
		},
		{
			'value': round10( values[ 2 ] ),
			'done': false
		},
		{
			'value': round10( values[ 3 ] ),
			'done': true
		}
	];
